...
#include "AstralPlayerStateBase/PlayerAttack/PlayerAttack.h"
#include "AstralPlayerStateBase/PlayerIdol/PlayerIdol.h"
...
public:
	...
	void ChangeState(std::shared_ptr<class AstralPlayerStateBase> pNewState)
	{
		// 1. 引数が空なら何もしない
		if (!pNewState) return;

		// 2. 現在のステートがあれば、終了処理（Exit）を呼ぶ
		if (m_pCurrentState)
		{
...
private:
	...
	std::shared_ptr<class AstralPlayerStateBase> m_pCurrentState;
