#include "AstroBoss_HPBar.h"
#include "Sprite2D/Sprite2D.h"
#include "DirectX/DirectX11.h"

AstroBoss_HPBar::AstroBoss_HPBar()
	: UIObject()
...
void AstroBoss_HPBar::Update()
{
	...
	// 🌟 修正ポイント：固定値 100.0f ではなく、ボスの最大HPを取得して計算する
	float currentHP = static_cast<float>(m_pTargetBoss->GetHP());
	float maxHP = static_cast<float>(m_pTargetBoss->GetMaxHP());
	...
	// これにより、最大HPがいくら増えてもバーの長さは枠内に収まります
...
void AstroBoss_HPBar::Create()
{
	...
	const float posY = static_cast<float>(WND_H - 400.0f);
