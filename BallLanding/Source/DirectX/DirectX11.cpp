...
void DirectX11::SetDepth(DEPTH_MODE mode)
{
	ID3D11DepthStencilState* pTmp = nullptr;

	switch (mode) {
	case DEPTH_MODE::OFF:    pTmp = m_pDepthStencilStateOff;    break;
	case DEPTH_MODE::ON:     pTmp = m_pDepthStencilStateOn;     break;
	case DEPTH_MODE::SKYBOX: pTmp = m_pDepthStencilStateSkybox; break;
	}
...
void DirectX11::Release()
{
	...
	SAFE_RELEASE(m_pDepthStencilStateSkybox);
...
HRESULT DirectX11::CreateDepthStencilState()
{
	...
	dsDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	dsDesc.DepthFunc = D3D11_COMPARISON_LESS; // 通常は「より手前」を描画
	dsDesc.StencilEnable = FALSE;
	m_pDevice11->CreateDepthStencilState(&dsDesc, &m_pDepthStencilStateOn);

	// --- 既存の OFF 設定 ---
	dsDesc.DepthEnable = FALSE;
	m_pDevice11->CreateDepthStencilState(&dsDesc, &m_pDepthStencilStateOff);

	// --- ★追加：スカイボックス用設定 (LESS_EQUAL) ---
	dsDesc.DepthEnable = TRUE;
	dsDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	// 背景(1.0)と重なっても描画を許可する
	dsDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;

	if (FAILED(m_pDevice11->CreateDepthStencilState(&dsDesc, &m_pDepthStencilStateSkybox))) {
		return E_FAIL;
	}
...
void DirectX11::ClearBackBuffer()
{
	...
	//レンダラーターゲットをリセット.
	DirectX11::GetInstance()->m_pContext11->OMSetRenderTargets(
		1,
		&DirectX11::GetInstance()->m_pBackBuffer_TexRTV,
		DirectX11::GetInstance()->m_pBackBuffer_DSTexDSV);
