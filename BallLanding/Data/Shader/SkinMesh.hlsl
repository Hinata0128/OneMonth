/***************************************************************************************************
*	SkinMeshCode Version 2.51 (Modified: Added Light Toggle)
...
cbuffer per_material : register(b1)
{
	...
	//--- 追加フラグ ---
    float g_IsLight; //ライト有効フラグ (1.0で有効、0.0で無効).
    float3 g_Padding; //16バイト境界合わせ用の詰め物.
};
...
float4 PS_Main(PSSkinIn input) : SV_Target
{
    ...
    if (g_IsLight < 0.5f)
    {
        return texColor; // ライト計算をスキップしてテクスチャ色を返す
    }
